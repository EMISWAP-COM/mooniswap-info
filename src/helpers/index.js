...
export function getPoolLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `${process.env.REACT_APP_INTERFACE_URL}/#/add/${token0Address}/${token0Address}`
  } else {
    return `${process.env.REACT_APP_INTERFACE_URL}/#/add/${token0Address}/${token1Address}`
  }
}

export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `${process.env.REACT_APP_INTERFACE_URL}/#/swap?inputCurrency=${token0Address}`
  } else {
    return `${process.env.REACT_APP_INTERFACE_URL}/#/swap?inputCurrency=${token0Address}&outputCurrency=${token1Address}`
  }
}
...
export function formattedPercent(percent, useBrackets = false) {
  ...
  if (percent < 0 && percent > -0.0001) {
    return (
      <Text fontWeight={500} color="#E85E59">
        {'< 0.0001%'}
      </Text>
    )
  }
  ...
  if (fixedPercent > 0) {
    if (fixedPercent > 100) {
      return <Text fontWeight={500} color="#37FFDB">{`+${percent?.toFixed(0).toLocaleString()}%`}</Text>
    } else {
      return <Text fontWeight={500} color="#37FFDB">{`+${fixedPercent}%`}</Text>
    }
  } else {
    return <Text fontWeight={500} color="#E85E59">{`${fixedPercent}%`}</Text>
  }
}
